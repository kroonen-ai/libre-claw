...
from benchmarks.coding_workflow.runner import TASK_IDS, prepare_fixture, run_suite, verify_fixture
...
async def test_fixed_coding_workflows_pass_offline_without_model_claims(tmp_path: Path) -> None:
    ...
    assert len(report["results"]) == len(TASK_IDS) + 1
    ...
    assert report["results"][-1]["checks"] == {"first_phase_preserved": True, "requirement_restored": True}
