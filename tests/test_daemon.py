...
async def test_daemon_serves_local_dashboard(monkeypatch, tmp_path: Path) -> None:
    ...
    assert 'const fallback = "libre";' in response.text
...
def test_dashboard_html_uses_config_theme_fallback() -> None:
    ...
    assert 'const fallback = "libre-light";' in dashboard_html(theme="light")
    assert 'const fallback = "libre";' in dashboard_html(theme="dark")
    assert 'const fallback = "libre";' in dashboard_html(theme="libre-default")
    assert 'const fallback = "libre";' in dashboard_html(theme="unknown-theme")
